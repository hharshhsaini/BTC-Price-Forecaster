...
# ══════════════════════════════════════════════════════════════════
# AUTOMATIC TRACKER - Runs in background to keep database updated
# ══════════════════════════════════════════════════════════════════
...
def auto_update_predictions():
    """Background task that updates predictions automatically"""
    ...
    try:
        ...
        # Fetch latest 500 klines from Binance
        binance_url = "https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1h&limit=500"
        ...
        # 2. BACKFILL missing predictions (last 168 hours)
        ALPHA = 0.01
        DF = 2.5
        T_MULT = 7.16
...
# Run auto-update in background thread (non-blocking)
if 'auto_update_started' not in st.session_state:
    st.session_state.auto_update_started = True
    thread = threading.Thread(target=auto_update_predictions, daemon=True)
    thread.start()
